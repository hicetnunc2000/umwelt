...
class IPFS:
    ...
    def listen(self, topic):
        sub = self.client.pubsub.subscribe(topic)
        for msg in sub:
            requests.post('http://localhost:5000/register', data=self.base64_decode(msg['data']))
...
class Onto():
    ...
    def insert_pub2(self, payload):
        ...
        pub.has_files_CID = payload['file_CID']
    ...
    # update jena
    def update(self):
        os.system("./../jena/bin/s-put http://localhost:3030/ontology/data default ./ontology/umwelt.owl")
    ...
    def search(self):
        ret = json.loads(
            subprocess.check_output("""./../jena/bin/s-query --service http://localhost:3030/ontology/query 'SELECT ?subject ?description ?date ?tags WHERE { OPTIONAL {?subject <http://127.0.0.1:8000/gott2.owl#has_CID> ?object.} OPTIONAL {?subject  <http://127.0.0.1:8000/gott2.owl#has_description> ?description.} OPTIONAL {?subject  <http://127.0.0.1:8000/gott2.owl#has_tags> ?tags.} OPTIONAL {?subject  <http://127.0.0.1:8000/gott2.owl#has_date> ?date.} BIND (STR(?subject) AS ?s) . BIND (STR(?tags) AS ?t) . OPTIONAL { FILTER CONTAINS ( LCASE(?s) , ENCODE_FOR_URI("philosophy"))} . OPTIONAL { FILTER CONTAINS ( LCASE(?t) , ENCODE_FOR_URI("philosophy"))} .}'""", shell=True)
        )
        return ret
