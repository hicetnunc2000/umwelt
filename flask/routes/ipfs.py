...
class Onto():
    ...
    def insert_pub2(self, payload):
        ...
        onto = get_ontology('./ontology/umwelt.owl')
        onto.load()
...
class SparqlQueries:
    ...
    def __init__(self):
        my_world = World()
        my_world.get_ontology("./ontology/umwelt.owl").load()   #path to the owl file
        #sync_reasoner(my_world)                                 #reasoner is started and synchronized
    ...
    def search(self, term):
        ...
        #Base URL
        term = term.lower()
        query =("SELECT DISTINCT ?cid WHERE { \
...
\
            OPTIONAL { FILTER CONTAINS ( LCASE(?s) , ENCODE_FOR_URI('%s'))} . \
            OPTIONAL { FILTER CONTAINS ( LCASE(?t) , ENCODE_FOR_URI('%s'))} . \
            OPTIONAL { FILTER CONTAINS ( LCASE(?d) , ENCODE_FOR_URI('%s'))} . \
\
